...
use app\store\model\user\Vip as VipModel;
...
class User extends UserModel
{
    ...
    private function getFilter(array $param = [])
    {
        ...
        $params['user_id'] > 0 && $filter[] = ['user_id', '=', (int)$params['user_id']];
        // 用户等级
        $params['gradeId'] > 0 && $filter[] = ['grade_id', '=', (int)$params['gradeId']];
        $params['is_inside'] > -1 && $filter[] = ['is_inside', '=', (int)$params['is_inside']];
        // 起止时间
    ...
    public function updateVip(array $data){
        // 变更前的等级id
        $oldVipId = $this['vip_id'];
        return $this->transaction(function () use ($oldVipId, $data) {
            $vip = $data['vip'];
            $vipId = isset($vip['vip_id'])?$vip['vip_id']:0;
            $planId = isset($vip['plan_id'])?$vip['plan_id']:0;
            $mobile = $data['mobile'];
            if($planId != 0){
                //获取充值套餐
                $plan = PlanModel::detail($planId);
                // 新增用户内部员工、vip套餐修改记录
                $vipLog = new VipLogModel();
                $vipLog->record([
                    'order_no'=>$vipLog->createOrderNo(),
                    'user_id' => $this['user_id'],
                    'plan_id' => $planId,
                    ...
                    'pay_time'=>time(),
                    'source' => 20,
                    'admin_user_id' => StoreUserService::getLoginUserId(),
                ]);
                if($vipId != 0){
                    //更新用户购买vip
                    $vip = new VipModel();
            ...
            // 更新用户是否内部员工、vip套餐
            $status = $this->save(['vip_id' => $vipId,'mobile'=>$mobile,'is_inside'=>$data['is_inside'],'domain'=>$data['domain']
            ,'is_jump'=>$data['is_jump'],'see_jump_num'=>$data['see_jump_num'],'jump_link'=>$data['jump_link'],'gengduo_link'=>$data['gengduo_link']
                ,'fanghui_link'=>$data['fanghui_link']
            ]);
